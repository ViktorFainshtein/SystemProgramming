...
{
	fsa_t *fsa = align((size_t)pool, sizeof(size_t));			/*fsa is aligned to pool*/
	char *free_list = NULL;
	size_t lost_data = 0;
	...
	assert(pool);
	if(pool_size < (size_t)align(block_size, sizeof(size_t)) + (size_t)align(sizeof(fsa_t), sizeof(size_t)) + 7)
	{
		return NULL;
	}
...
size_t FSASuggestSize(size_t block_size, size_t num_of_blocks)
{
	...
	res = (block_size * num_of_blocks) + struct_size + 7;
